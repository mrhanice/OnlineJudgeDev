...
class ChooseJudgeServer:
    ...
    #选择一个当前server的task_number < cpu_core*2的server来分配这次task
...
class JudgeDispatcher(DispatcherBase):
    def __init__(self, submission_id, problem_id):
        super().__init__()
        self.submission = Submission.objects.get(id=submission_id) #获取submission
        self.contest_id = self.submission.contest_id #获取contest_id
        self.last_result = self.submission.result if self.submission.info else None #获取上次的提交结果，如果没有就是None
    ...
    #judge
    def judge(self):
        language = self.submission.language #获取到语言
        sub_config = list(filter(lambda item: language == item["name"], SysOptions.languages))[0] #获取语言选项
        spj_config = {}
        if self.problem.spj_code: #如果是spj的代码
